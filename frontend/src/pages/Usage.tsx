...
const COLORS = {
  hotlist: 'rgb(var(--color-accent))',
  rss: 'rgb(var(--color-positive))',
  matched: 'rgb(var(--color-link))',
  pushed: 'rgb(var(--color-accent))',
  grid: 'var(--chart-grid)',
  axis: 'var(--chart-axis)',
  crosshair: 'rgb(var(--color-accent))',
};
...
function BloombergTooltip({ active, payload, label }: { active?: boolean; payload?: Array<{ dataKey: string; value: number; color: string }>; label?: string }) {
  ...
  return (
    <div className="bg-card border border-accent/60 px-2 py-1 text-xs font-mono">
...
export default function Usage() {
  ...
  const axisStyle = { fill: COLORS.axis, fontSize: 9, fontFamily: 'JetBrains Mono, monospace' };
    ...
    <div className="space-y-2">
        ...
        <div className="bb-panel">
          ...
          <div className="bb-panel-body text-xs space-y-2">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
              {Object.entries(usage.limits).map(([action, limit]) => {
                const count = usage.usage[action]?.count ?? 0;
                const pct = limit > 0 ? Math.min(100, Math.round((count / limit) * 100)) : 0;
                return (
                  <div key={action} className="border border-border p-2">
                    <div className="text-accent/70 mb-1">{action.toUpperCase()}</div>
                    <div className="text-accent font-bold">{count} / {limit}</div>
      ...
      {data.length > 0 && (
        <div className="bb-panel">
          ...
          <div className="bb-panel-body" style={{ background: 'linear-gradient(180deg, var(--chart-bg-top) 0%, var(--chart-bg-bottom) 100%)' }}>
            <ResponsiveContainer width="100%" height={300}>
              <AreaChart data={data} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                ...
                <XAxis dataKey="run" tick={axisStyle} axisLine={{ stroke: 'rgb(var(--color-border))' }} tickLine={false} />
                <YAxis tick={axisStyle} axisLine={false} tickLine={false} />
                <Tooltip content={<BloombergTooltip />} cursor={{ stroke: COLORS.crosshair, strokeWidth: 1, strokeDasharray: '3 3' }} />
                <Legend wrapperStyle={legendStyle} formatter={(v: string) => <span style={{ color: COLORS.axis, fontSize: 9 }}>{LABEL_ZH[v] || v}</span>} />
                ...
                <Area type="monotone" dataKey="hotlist" stroke={COLORS.hotlist} strokeWidth={1.5} fill="url(#gradHotlist)" dot={false} activeDot={{ r: 3, stroke: COLORS.hotlist, fill: 'rgb(var(--color-bg))' }} />
                <Area type="monotone" dataKey="rss" stroke={COLORS.rss} strokeWidth={1.5} fill="url(#gradRss)" dot={false} activeDot={{ r: 3, stroke: COLORS.rss, fill: 'rgb(var(--color-bg))' }} />
                <Area type="monotone" dataKey="matched" stroke={COLORS.matched} strokeWidth={1.5} fill="url(#gradMatched)" dot={false} activeDot={{ r: 3, stroke: COLORS.matched, fill: 'rgb(var(--color-bg))' }} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
